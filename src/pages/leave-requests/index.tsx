...
import { useRoqClient, useLeaveRequestFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<LeaveRequestInterface, unknown>;

interface LeaveRequestListPageProps {
...
export function LeaveRequestListPage(props: LeaveRequestListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useLeaveRequestFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.leave_request.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: LeaveRequestInterface) => {
    if (hasAccess('leave_request', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/leave-requests/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/leave-requests/view/${record.id}`} passHref legacyBehavior>
                <Button
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="leave_request" />
        </Flex>
