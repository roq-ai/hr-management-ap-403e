...
import { useRoqClient, useLeaveRequestFindFirst } from 'lib/roq';
...
function LeaveRequestEditPage() {
  ...
  const { data, error, isLoading, mutate } = useLeaveRequestFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: LeaveRequestInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.leave_request.update({
        data: values as RoqTypes.leave_request,
        where: {
          id,
        },
      });
