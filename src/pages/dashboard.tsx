...
import { useRoqPlatformUserProfiles, useEmployeeCount, useLeaveRequestCount, useJobPositionCount } from 'lib/roq';

import { LeaveRequestListPage as ShowCaseList } from 'pages/leave-requests';
...
function DashboardPage() {
  ...
  const leaveRequestStat = useLeaveRequestCount();

  const jobPositionStat = useJobPositionCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: leaveRequestStat?.data,
      isLoading: leaveRequestStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Job Positions',
      result: jobPositionStat?.data,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`leave-requests`} passHref legacyBehavior>
                  <Button
